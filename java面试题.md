...
* [java面试题](#java面试题)
    * [Java 面向对象](#java-面向对象)
      ...
      * [52、Set 里的元素是不能重复的，那么用什么方法来区分重复与否呢? 是用==还是equals()? 它们有何区别?用 contains 来区分是否有重复的对象。还是都不用。](#52set-里的元素是不能重复的那么用什么方法来区分重复与否呢-是用还是equals-它们有何区别用-contains-来区分是否有重复的对象还是都不用)
